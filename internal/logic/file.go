...
func (l *GetFileContentLogic) GetFileContent(req *types.FileContentRequest) (string, error) {
	...
	if errors.Is(err, model.ErrNotFound) {
		return types.EmptyString, errs.NewRecordNotFoundErr(types.NameCodeBase, fmt.Sprintf("client_id: %s, clientCodebasePath: %s", clientId, clientCodebasePath))
	}
	if err != nil {
	...
	if utils.IsBlank(codebasePath) {
		return types.EmptyString, errors.New("codebase path is empty")
	}
