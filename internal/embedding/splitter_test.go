...
`)
	...
	assert.Equal(t, 5, chunks[0].Range[0], "main function should start at line 5")
	...
	assert.Equal(t, 9, chunks[1].Range[0], "anotherFunc should start at line 9")
	...
	for _, chunk := range chunks {
		...
		assert.Contains(t, string(chunk.Content), "}", "Each chunk should contain function body")
		// 验证行号范围
		assert.GreaterOrEqual(t, chunk.Range[0], 0, "Chunk should have valid start line (0-based)")
		assert.GreaterOrEqual(t, chunk.Range[2], chunk.Range[0], "Chunk should have valid line range")
	}
}
			...
			verifyOverlap: func(t *testing.T, chunks []*types.CodeChunk) {
				...
				for funcName, funcChunks := range funcChunks {
					...
					for i := 1; i < len(funcChunks); i++ {
						...
						// 1. 验证行号重叠
						assert.Lessf(t, curr.Range[0], prev.Range[2],
							"Chunk %d (%d-%d) should overlap with chunk %d (%d-%d) in function %s",
							i, curr.Range[0], curr.Range[2], i-1, prev.Range[0], prev.Range[2], funcName)
