...
func (t *task) Execute() {
	...
	t.logger.Infof("start to execute embedding task %v", t.msg)
	start := time.Now()

	err := func(t *task) error {
		...
		for k, v := range syncFileList {
			...
			// timeout
			case <-t.ctx.Done():
				t.logger.Errorf("embedding task %d timeout", t.taskHistoryId)
				return errs.RunTimeout
			default:
				if err = t.processFile(&types.SyncFile{Path: k, Op: types.FileOp(v)}); err != nil {
					t.logger.Errorf("update embedding task file %s failed: %v", k, err)
					failedFileCnt++
				} else {
					successFileCnt++
				}
	...
	t.logger.Infof("embedding task end successfully, cost: %d s, : %v", time.Since(start), t.msg)
}

func (t *task) updateTaskSuccess() {
	...
	if err := t.svcCtx.IndexHistoryModel.Update(t.ctx, m); err != nil {
		// 任务已经成功
		t.logger.Errorf("update embedding task history %d failed: %v, model:%v", t.msg.CodebaseID, err, m)
	}
}

func (t *task) handleIfTaskFailed(err error) bool {
	if err != nil {
		...
		if err != nil {
			t.logger.Errorf("update embedding task history failed: %v", t.msg.CodebaseID, err)
		}
		return true
	}
	return false
...
func (t *task) initTaskHistory() error {
	// 插入一条任务记录
	...
	if _, err := t.svcCtx.IndexHistoryModel.Insert(t.ctx, embedTaskHistory); err != nil {
		t.logger.Errorf("insert task history %v failed: %v", embedTaskHistory, err)
...
func (t *task) getSyncFileListCollapse() (map[string]string, []string, error) {
	// 根据元数据获取代码文件列表
	var syncFileList map[string]string

	// 递归目录，进行处理，并发
	// 获取代码文件列表
	list, err := t.svcCtx.CodebaseStore.List(t.ctx, t.msg.CodebaseID, types.SyncMedataDir, types.ListOptions{})
	if err != nil {
		return nil, nil, err
	}
	if len(list) == 0 {
		return nil, nil, errors.New("task metadata dir is empty")
	}
	...
	for it.Next() {
		...
		syncMetaData, err := t.svcCtx.CodebaseStore.Read(t.ctx, t.msg.CodebaseID, filepath.Join(types.SyncMedataDir, metadataFile), types.ReadOptions{})
		if err != nil {
...
func (t *task) processFile(syncFile *types.SyncFile) error {
...
func (t *task) processAddFile(syncFile *types.SyncFile) error {
	t.logger.Debugf("process add file %v", syncFile)
	file, err := t.svcCtx.CodebaseStore.Read(t.ctx, t.msg.CodebaseID, syncFile.Path, types.ReadOptions{})
	if err != nil {
		return err
	}
	// 切分文件
...
func (t *task) processDeleteFile(file *types.SyncFile) error {
	del := []*types.CodeChunk{
		{
			CodebaseId:   t.msg.CodebaseID,
			CodebasePath: t.msg.CodebasePath,
...
func (t *task) deleteProcessedSyncMetadata(metadataFileNames []string) {
	for _, n := range metadataFileNames {
		if err := t.svcCtx.CodebaseStore.Delete(t.ctx, t.msg.CodebaseID, filepath.Join(types.SyncMedataDir, n)); err != nil {
