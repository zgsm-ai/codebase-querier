...
func NewEmbeddingTask(ctx context.Context, svcCtx *svc.ServiceContext, msg *types.CodebaseSyncMessage) Task {
...
func (t *embeddingTask) Run() {

	t.logger.Infof("start to execute embedding embeddingTask %v", t.msg)
	start := time.Now()

	err := func(t *embeddingTask) error {
		...
		for k, v := range syncFileList {
			...
			// timeout
			case <-t.ctx.Done():
				t.logger.Errorf("embedding embeddingTask %d timeout", t.taskHistoryId)
				return errs.RunTimeout
			default:
				if err = t.processFile(&types.SyncFile{Path: k, Op: types.FileOp(v)}); err != nil {
					t.logger.Errorf("update embedding embeddingTask file %s failed: %v", k, err)
					failedFileCnt++
				} else {
					successFileCnt++
				}
		...
		// update embeddingTask when success
		t.updateTaskSuccess()
	...
	t.logger.Infof("embedding embeddingTask end successfully, cost: %d s, : %v", time.Since(start), t.msg)
}

func (t *embeddingTask) updateTaskSuccess() {
	...
	if err := t.svcCtx.IndexHistoryModel.Update(t.ctx, m); err != nil {
		// 任务已经成功
		t.logger.Errorf("update embedding embeddingTask history %d failed: %v, model:%v", t.msg.CodebaseID, err, m)
	}
}

func (t *embeddingTask) handleIfTaskFailed(err error) bool {
	if err != nil {
		...
		if err != nil {
			t.logger.Errorf("update embedding embeddingTask history failed: %v", t.msg.CodebaseID, err)
		}
		return true
	}
	return false
...
func (t *embeddingTask) initTaskHistory() error {
	// 插入一条任务记录
	...
	if _, err := t.svcCtx.IndexHistoryModel.Insert(t.ctx, embedTaskHistory); err != nil {
		t.logger.Errorf("insert embeddingTask history %v failed: %v", embedTaskHistory, err)
...
func (t *embeddingTask) getSyncFileListCollapse() (map[string]string, []string, error) {
	// 根据元数据获取代码文件列表
	...
	if len(list) == 0 {
		return nil, nil, errors.New("embeddingTask metadata dir is empty")
	}
...
func (t *embeddingTask) processFile(syncFile *types.SyncFile) error {
...
func (t *embeddingTask) processAddFile(syncFile *types.SyncFile) error {
...
func (t *embeddingTask) processDeleteFile(file *types.SyncFile) error {
	del := []*types.CodeChunk{
		{
			CodebaseId:   t.msg.CodebaseID,
			CodebasePath: t.msg.CodebasePath,
...
func (t *embeddingTask) deleteProcessedSyncMetadata(metadataFileNames []string) {
